...
public class TokenRevocationLiveTest {
    ...
    public void givenUser_whenRevokeToken_thenTokenInvalidError() {
        ...
        final Response tokenResponse1 = RestAssured.given().header("Authorization", "Bearer " + accessToken2).get("http://localhost:8082/ems-api/tokens");
        assertEquals(200, tokenResponse1.getStatusCode());

        final Response revokeResponse = RestAssured.given().header("Authorization", "Bearer " + accessToken1).post("http://localhost:8082/ems-api/tokens/revoke/" + accessToken2);
        assertEquals(200, revokeResponse.getStatusCode());

        final Response tokenResponse2 = RestAssured.given().header("Authorization", "Bearer " + accessToken2).get("http://localhost:8082/ems-api/tokens");
    ...
    public void givenUser_whenRevokeRefreshToken_thenRefreshTokenInvalidError() {
        ...
        authorizeClient("fooClientIdPassword");
        final Response refreshTokenResponse = RestAssured.given().header("Authorization", "Bearer " + accessToken3).get("http://localhost:8082/ems-api/tokens");
        assertEquals(200, refreshTokenResponse.getStatusCode());

        final Response revokeRefreshTokenResponse = RestAssured.given().header("Authorization", "Bearer " + accessToken1).post("http://localhost:8082/ems-api/tokens/revokeRefreshToken/" + refreshToken);
        ...
        authorizeClient("fooClientIdPassword");
        final Response refreshTokenResponse2 = RestAssured.given().header("Authorization", "Bearer " + accessToken4).get("http://localhost:8082/ems-api/tokens");
